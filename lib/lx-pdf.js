...
module.exports = function (template) {
    ...
    function setUpPage(document, layout) {
        ...
        if (showFooter === true && pageFooter && pageFooter.data) {
            var pageFooterHeight = getValue('format.height', 60, pageFooter),
                pageFooterData = pageFooter.page ? (pageFooter.data + (pageIndex + 1).toString()) : pageFooter.data || '',
                pageFooterFormat = {};

            pageFooterData += pageFooter.data2 || ''; //if data2 exists then append
    ...
    function addTextToDocument(document, sectionData, sectionInfo) {
        ...
        if (typeof text === 'string') {
            ...
            for (var charIndex = 0; charIndex < text.length; charIndex++) {
                switch (text.charAt(charIndex)) {
                    ...
                    case '\n':
                        ...
                        currentLineWidth = 0;
                        lines.push({text: text.slice(lineStartIndex, charIndex).trim(), start: lastLineBreakIndex, end: charIndex});
                    ...
                    case ' ':
                        ...
                        if ((currentLineWidth + currentWordWidth) >= pixelWidth) {
                            ...
                            currentLineIndex++;
                            lines.push({text: text.slice(lineStartIndex, lastLineBreakIndex).trim(), start: lastLineBreakIndex, end: charIndex});
            ...
            if (splitIndex === -1) {
                ...
                lines.push({text: text.slice(lineStartIndex, charIndex).trim(), start: lineStartIndex, end: charIndex});
            } else {
            ...
            var sectionMaxY =  (sectionDimension.top + sectionDimension.height) - currentLineHeight;

            for (var i = 0; i < lines.length; i++) {
                if (linePositionY >= sectionMaxY) {
                    result.done = false;

                    if (i > 0 && typeof result.data === 'string') {
                        result.data = result.data.slice(lines[i - 1].start);
                    }
                ...
                currentLineHeight = writeText(document, linetext, sectionDimension.left, linePositionY, format).y - linePositionY;

                linePositionY = document.y;
                result.topIndex = (document.y - sectionDimension.top);
            }
        }
    ...
    function addTableToDocument(document, sectionData, sectionInfo, newPageCallback) {
        ...
        var templateOptions = getValue('options.sections.' + sectionData, undefined);

        while (templateOptions.format.breakPage && globalPageIndexCounter > sectionInfo.pageIndex) { //multiple table sections must be aligned under the previous
            sectionInfo.topIndex = 0; // New Page, reset Line
            setPage(document, ++pageIndex);
            sectionInfo.pageIndex++;
        }

        if (typeof sectionData === 'string') { //break page if no space for table and breakPage is defined
            if (templateOptions.format.breakPage && typeof templateOptions.format.breakPage == 'number') //pixels to the end of a section
                if ((document.page.height - (sectionInfo.topIndex || 0)) < templateOptions.format.breakPage) {
                    setPage(document, ++pageIndex);
                    sectionInfo.topIndex = 0; // New Page, reset Line
                    sectionInfo.pageIndex++;
        ...
        lxHelpers.arrayForEach(tableInfo, function (cell) {
            if (cell.width > 0) {
                maxTableWidth -= Math.floor(cell.width);
                numAutoCalcCells--;
            }
        });
        ...
        var defaultCellWidth = (numAutoCalcCells > 0) ? Math.floor(maxTableWidth / numAutoCalcCells) : 0,
            tableWidth = 0;
        ...
        for (var idxTableRow = 0; idxTableRow < tableData.length; idxTableRow++) {
            ...
            for (var idxTableCol = 0; idxTableCol < currentRowData.length; idxTableCol++) {
                ...
                if (typeof currentRowData[idxTableCol] === 'string') {
                    ...
                    rowEntry.format.x_offset = currentRowData[idxTableCol].x_offset || undefined;
                    rowEntry.format.y_offset = currentRowData[idxTableCol].y_offset || undefined;
        ...
        for (var idxRow = 0; idxRow < tableContentData.length; idxRow++) {
            ...
            lxHelpers.forEach(tableContentData[idxRow], function (cellData) {
                ...
                if (cellData.format.align in cellOptions) {
                    x_offset = cellData.format && cellData.format.x_offset ? cellData.format.x_offset : cellOptions[cellData.format.align].x_offset;
                }

                // Render cell
                var currentCellHeight = (writeText(document, cellData.text, currentCellPositionLeft + x_offset, 
                    (cellData.format && cellData.format.y_offset ? (currentCellPositionTop + cellData.format.y_offset + 1) : (currentCellPositionTop + 1)), cellData.format).y - currentCellPositionTop) + 0.75;
                ...
                if (cellData.border || (usePrevBorder && cfgPrevBorder)) {
                    ...
                    lxHelpers.forEach(cfgPrevBorder.position || [], function (borderposition) {
                        ...
                        if (borderRules[borderposition]) {
                            ...
                            if (borderStyle === 'normal' || borderStyle === 'thin') {
